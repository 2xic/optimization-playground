...
class MoE(nn.Module):
    def __init__(self, config: Config, num_experts=4, top_k=2):
        ...
        self.experts = nn.ModuleList([
            Model(config) for _ in range(num_experts)
        ])
        self.config = config
    ...
    def forward(self, x):
        ...
        outputs = torch.zeros(batch_size, self.config.vocab_size, self.config.sequence_length, device=x.device)
        ...
        for i, expert in enumerate(self.experts):
            ...
            if expert_mask.any():
                ...
                expert_outputs = expert(inputs_to_expert.long()).view(batch_size, self.top_k, -1)                
