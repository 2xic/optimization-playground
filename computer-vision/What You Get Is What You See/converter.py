"""
Based off https://github.com/RobinXL/inkml2img/blob/master/inkml2img.py
"""
import numpy as np
...
def get_traces_data(inkml_file_abs_path, xmlns='{http://www.w3.org/2003/InkML}'):
    ...
    root = tree.getroot()
    decoded = (root.findall(xmlns + 'annotation')[0].text)
#    exit(0)
...
def inkml2img(input_path, output_path, color='black'):
    traces, decoded = get_traces_data(input_path)
    for elem in traces:
        ls = elem['trace_group']
        for subls in ls:
            data = np.array(subls)
...
if __name__ == "__main__":
    ...
    import json
    dataset = {}
    for i in glob.glob("/home/brage/optimization-playground/computer-vision/What You Get Is What You See/dataset/CROHME2013_data/TrainINKML/HAMEX/*.inkml")[:100]:
        name = os.path.basename(i).replace(".inkml", "")
        out = f"./dataset/dataset_png/{name}.png"
        decoded = inkml2img(i, out, color='#284054')
        if "$" in decoded:
            dataset[out] = decoded
