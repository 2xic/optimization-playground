# copied from https://github.com/daejunpark/evm-disassembler/blob/master/byte2op.py
...
def decode(hexcode):
    ...
    pushcnt = 0
    cnt = -1
    for item in range(0, len(hexcode), 2):
        item = hexcode[item:item+2]
        #print(item)
        cnt += 1
        if pushcnt > 0:
            h += item.lower()
            ...
            #o += pc(cnt, size) + ' ' + item.lower() + ' ' + 
            o += opcodes[item.lower()]
