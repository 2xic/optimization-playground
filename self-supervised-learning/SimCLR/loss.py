...
class Loss:
	...
	def slow_loss(self, z_1, z_2):
		...
		loss_value = 0
		for i in range(1, N):
			# slow_l_i_j(2 * i - 1, 2 * i) + slow_l_i_j(2 * i, 2 * i - 1)
			loss_value += loss_compute(i)
	...
	def debug_calc_similarity_batch(self, a, b):
		"""
		Note: this is where the bug was in the trainining loop
		"""
...
class ContrastiveLoss(torch.nn.Module):
	...
	def forward(self, proj_1, proj_2):
		...
		nominator = torch.exp(positives / self.temperature)
		denominator = self.mask.to(
			similarity_matrix.device) * torch.exp(similarity_matrix / self.temperature)
